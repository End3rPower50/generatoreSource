...
if (isset($_POST['flagalt']) && isset($_POST['generator'])){
    $fig = mysqli_real_escape_string($con, $_POST['flagidgen']);
	$generatorid = mysqli_real_escape_string($con, $_POST['generator']);
    mysqli_query($con, "UPDATE `generator$generatorid` SET `status` = '2' WHERE `id` = '$fig'") or die(mysqli_error($con)); 
}
                ...
                <ul class="nav pull-right top-menu">
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu extended logout">
                            ...
                            <li><a href="changepassword.php"><i class="icon-cog"></i> Settings</a></li>
							  ...
							  <section class="panel">
								  <div class="panel-body">
									  ...
									  <form id="flagform'.$row["id"].'" action="generator.php" method="POST">
                                          ...
                                          <input type="hidden" id="flagidgen'.$row["id"].'" name="flagidgen" value="">
			...
			$(document).ready(function(){
				$("#generate'.$row["id"].'").click(function(){
				 $.get("lib/generate.php?generator='.$row["id"].'&username='.$username.'", function(response){
					...
					$("#generator'.$row["id"].'").val(response[0]);
					$("#flag'.$row["id"].'").val(response[0]);
                    $("#flagidgen'.$row["id"].'").val(response[1]);
				 });
				});
			});
